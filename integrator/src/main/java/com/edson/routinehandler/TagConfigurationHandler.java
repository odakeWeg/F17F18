package com.edson.routinehandler;
...
public class TagConfigurationHandler {
    ...
    private ComunicacaoSerial gambiarra;

    public void reset(){
        resultSequence = new LinkedHashMap<>();
    }

    public String executeTagFunction(Node node) {
    ...
    private String readCommand(int address, int[] registers, int timeOut, int waitBefore, int waitAfter) {
    ...
    private String writeCommand(int address, int[] registers, int timeOut, int waitBefore, int waitAfter) {
    ...
    private String startConnectionCommand(String portName, int baudRate, int dataBits, int stopBits, String parity, int timeout, String protocol) {
        switch (protocol) {
            case "modbus":
